...
class LanguageService {
    static getAllLanguageIds() {
        return Array.from(this.languages.keys());
    }
    static getLanguageConfig(languageId) {
        return this.languages.get(languageId);
    }
    static generatePrompt(language, type, code, instruction) {
...
\`\`\`${language}`;
    ...
    static getCompletionPatterns(language) {
        const config = this.languages.get(language);
        return Object.fromEntries(Object.entries(config?.completionPatterns || {}).filter(([_, value]) => value !== undefined));
    }
    static getTestImports(language) {
        const config = this.languages.get(language);
...
LanguageService.languages = new Map([
    ['javascript', {
            id: 'javascript',
