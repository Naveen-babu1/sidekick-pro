...
const os = require("os");
const LanguageService_1 = require("../languages/LanguageService");
...
class LocalAIProvider {
    ...
    async initializeLlamaCpp() {
        ...
        if (!modelPath) {
            vscode.window.showErrorMessage("Failed to initialize llama.cpp model. Please check the model path.");
            return;
        }
    ...
    async runFirstTimeSetup() {
        const result = await vscode.window.showInformationMessage("🚀 Welcome to Sidekick AI! Let's set it up (one-time setup, 2 minutes)", "Start Setup", "Skip");
        ...
        vscode.window.showInformationMessage("✅ Setup complete! Sidekick AI will now start automatically whenever you use it.");
        return true;
    }
    async setupLlamaCpp() {
        ...
        if (autoDetected) {
            const useDetected = await vscode.window.showInformationMessage(`Found llama.cpp at: ${autoDetected}. Use this?`, "Yes", "Browse for another");
            if (useDetected === "Yes") {
                return autoDetected;
            }
        }
        // If not found, guide user
        const action = await vscode.window.showInformationMessage("llama.cpp is required to run AI models locally. Do you have it installed?", "I have it - let me select it", "Download it", "Help");
        if (action === "I have it - let me select it") {
            ...
            await vscode.window.showInformationMessage("After downloading and extracting llama.cpp, click OK", "OK");
            return this.setupLlamaCpp(); // Recursive call to select after download
        }
        return null;
    }
    async autoDetectLlamaCpp() {
        const possiblePaths = [
            // Windows paths
        ...
        // Try to find in PATH
        try {
            const { stdout } = await (0, util_1.promisify)(child_process_1.exec)(process.platform === "win32" ? "where llama-server" : "which llama-server");
            return stdout.trim();
        }
        catch {
            return null;
    ...
    async setupModel() {
        const action = await vscode.window.showInformationMessage("Now you need an AI model file (GGUF format, ~1-2GB)", "I have a model", "Download recommended model", "Show options");
        if (action === "I have a model") {
            ...
            await vscode.window.showInformationMessage("Download the model file (1GB), save it anywhere, then click OK", "OK");
    ...
    async ensureServerRunning() {
        ...
        // Get configuration
        const config = vscode.workspace.getConfiguration("sidekick-ai");
        let modelPath = config.get("modelPath");
        let llamaPath = config.get("llamaPath");
        if (!modelPath || !llamaPath) {
            ...
            modelPath = configUpdated.get("modelPath");
            llamaPath = configUpdated.get("llamaPath");
            if (!modelPath || !llamaPath) {
                return false;
            }
        }
    ...
    async startServer(llamaPath, modelPath) {
        if (this.serverStartAttempts >= this.maxStartAttempts) {
            vscode.window.showErrorMessage("Failed to start AI server after multiple attempts. Please check your configuration.");
            return false;
        }
        ...
        try {
            const config = vscode.workspace.getConfiguration("sidekick-ai");
            const port = config.get("port") || 8080;
            const contextSize = config.get("contextSize") || 4096;
            const useGpu = config.get("useGpu") || false;
            const args = [
                "-m", modelPath,
            ...
            // Start new process
            this.llamaProcess = (0, child_process_1.spawn)(llamaPath, args, {
            ...
            this.llamaProcess.on("error", (error) => {
                ...
                this.isServerRunning = false;
                vscode.window.showErrorMessage(`Failed to start AI server: ${error.message}`);
            });
    ...
    async ensureModel() {
        const config = vscode.workspace.getConfiguration("sidekick-ai");
        const knownModelPath = config.get("modelPath");
        if (!knownModelPath) {
        ...
        if (knownModelPath) {
            try {
                ...
                vscode.window.showErrorMessage("Configured model path no longer exists");
    ...
    async setupModelPath() {
        const result = await vscode.window.showInformationMessage("No AI model configured. Sidekick AI needs a GGUF model file to work.", "Select Model File", "Download Model", "Setup Guide");
        if (result === "Select Model File") {
            ...
            vscode.env.openExternal(vscode.Uri.parse("https://huggingface.co/Qwen/Qwen2.5-Coder-1.5B-Instruct-GGUF/resolve/main/qwen2.5-coder-1.5b-instruct-q4_k_m.gguf"));
            vscode.window.showInformationMessage("Download the model file, then come back and select it using 'Select Model File'");
        }
    ...
    async startLlamaCppServer() {
        ...
        try {
            ...
            // Start the server
            this.llamaProcess = (0, child_process_1.spawn)(serverPath, args);
    ...
    async findLlamaExecutable() {
        const config = vscode.workspace.getConfiguration("sidekick-ai");
        const configuredPath = config.get("llamaPath");
        if (configuredPath) {
            try {
                ...
                vscode.window.showWarningMessage("Configured llama.cpp path not found. Please reconfigure.");
            }
        }
        ...
        for (const cmd of execNames) {
            try {
                const { stdout } = await execAsync(platform === "win32" ? `where ${cmd}` : `which ${cmd}`);
        ...
        const result = await vscode.window.showErrorMessage("llama.cpp not found. You need to install it for Sidekick AI to work.", "Download llama.cpp", "Select Executable", "Setup Instructions");
        if (result === "Download llama.cpp") {
            const downloadUrl = platform === "win32"
                ? "https://github.com/ggerganov/llama.cpp/releases"
                : "https://github.com/ggerganov/llama.cpp#build";
            vscode.env.openExternal(vscode.Uri.parse(downloadUrl));
            vscode.window.showInformationMessage("After downloading, extract it and select the llama-server executable");
        }
    ...
    async generateCompletion(prompt, context, maxTokens = 150, languageId) {
...
${currentLine}`;
    ...
    cleanCompletion(completion, currentLine) {
        // Remove FIM tokens
    ...
    getQuickCompletion(prompt, languageId) {
        const trimmed = prompt.trim();
        const lastLine = trimmed.split("\n").pop() || "";
        // Get language-specific patterns
        const language = languageId || this.detectLanguage(prompt);
        const patterns = LanguageService_1.LanguageService.getCompletionPatterns(language);
    ...
    async downloadModels(progress, token) {
        progress.report({
            message: "Please download GGUF models manually",
            increment: 100,
        });
    }
    async explainCode(code, context, languageId) {
        if (!this.isServerRunning) {
            await this.ensureServerRunning();
        }
        const language = languageId || this.detectLanguage(code);
        const prompt = LanguageService_1.LanguageService.generatePrompt(language, 'explain', code, "");
        try {
            const response = await fetch(`${this.llamaEndpoint}/completion`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
    ...
    async refactorCode(code, instruction, context, languageId) {
        if (!this.isServerRunning) {
            await this.ensureServerRunning();
        }
        const language = languageId || this.detectLanguage(code);
        const prompt = LanguageService_1.LanguageService.generatePrompt(language, 'refactor', code, instruction);
        try {
            const response = await fetch(`${this.llamaEndpoint}/completion`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
    ...
    async generateTests(code, context, languageId) {
        if (!this.isServerRunning) {
            await this.ensureServerRunning();
        }
        const language = languageId || this.detectLanguage(code);
        const prompt = LanguageService_1.LanguageService.generatePrompt(language, 'test', code, "");
        try {
            const response = await fetch(`${this.llamaEndpoint}/completion`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
...
Assistant:`;
    ...
    async streamChat(message, context, onToken) {
        // Not implemented for llama.cpp yet
    }
    async initialize() {
        ...
        const llamaPath = config.get("llamaPath");
        try {
            ...
            if (this.isServerRunning) {
                ...
                const result = await vscode.window.showWarningMessage("Sidekick AI needs setup. Would you like to configure it now?", "Setup Now", "Later");
                if (result === "Setup Now") {
                    vscode.commands.executeCommand("workbench.action.openSettings", "@ext:NaveenBabu.sidekick-ai");
                }
            }
        }
        catch (error) {
    ...
    detectLanguage(code) {
        return LanguageService_1.LanguageService.detectLanguage(code);
    }
    getTestFramework(language) {
        const config = LanguageService_1.LanguageService.getLanguageConfig(language);
