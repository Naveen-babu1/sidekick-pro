...
exports.CodeFixHandler = void 0;
const vscode = require("vscode");
class CodeFixHandler {
    constructor(localAI, context) {
        this.localAI = localAI;
        this.context = context;
    }
    async fixError(document, diagnostic) {
...
Analyze the entire file context and provide ONLY the fixed code that should replace "${errorText}". Do not include explanations, just the corrected code:`;
    ...
    findContainingBlock(document, errorLine) {
    ...
    showFixPreview(editor, replaceRange, fixedCode, originalCode) {
        // Store document URI to get fresh editor later
        const documentUri = editor.document.uri;
        const documentVersion = editor.document.version;
        // Store the range in closure so it can't be changed
        const rangeToReplace = new vscode.Range(replaceRange.start.line, replaceRange.start.character, replaceRange.end.line, replaceRange.end.character);
        // Create a webview panel to show the fix preview
        const panel = vscode.window.createWebviewPanel("sidekickFix", "AI Code Fix", vscode.ViewColumn.Two, {
            enableScripts: true,
        ...
        panel.webview.onDidReceiveMessage(async (message) => {
            switch (message.command) {
                case "accept":
                    ...
                    const currentEditor = vscode.window.visibleTextEditors.find((e) => e.document.uri.toString() === documentUri.toString());
                    if (!currentEditor) {
    ...
    getFixPreviewHtml(original, fixed) {
        return `<!DOCTYPE html>
        <html lang="en">
        <head>
            <meta charset="UTF-8">
...
exports.CodeFixHandler = CodeFixHandler;
//# sourceMappingURL=CodeFixHandler.js.map